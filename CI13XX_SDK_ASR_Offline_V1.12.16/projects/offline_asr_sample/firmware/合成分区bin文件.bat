...
if %fmt% == 1 (
	...
	for /d %%i in (*) do (
		echo %%i | %SDK_PATH%\tools\build-tools\bin\busybox grep -E "\[.*\].*" > nul && call:to_adpcm adpcm %%i
	)
	...
	for /d %%i in (*) do (
		echo %%i | %SDK_PATH%\tools\build-tools\bin\busybox grep -E "\[.*\].*" > nul && call:to_flac flac %%i
	)
	...
	for /d %%i in (*) do (
		echo %%i | %SDK_PATH%\tools\build-tools\bin\busybox grep -E "\[.*\].*" > nul && call:to_pcm pcm %%i
	)
