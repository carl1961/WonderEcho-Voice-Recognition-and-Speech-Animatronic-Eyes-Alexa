...
#define CWSL_WAKEUP_NUMBER 1                    // 可学习的唤醒词数量
#define WAKE_UP_ID 3                             // 学习的唤醒词对应的命令词ID
#define CWSL_CMD_NUMBER ((sizeof(reg_cmd_list) / sizeof(reg_cmd_list[0])))     // 可学习的命令词数量

// 学习时 每个词需说几遍，默认 1 遍即可,支持1、2遍,最大支持 2 遍;
#define CWSL_REG_TIMES              (1) 

// 自学习模板的最小字数，默认 2 个字，可设置模板最小 2 、3 个字;           
#define CWSL_TPL_MINWORD            (2) 

// 学习的唤醒词阈值门限，越小越灵敏，默认 37, 最小可配置到 32;
#define CWSL_WAKEUP_THRESHOLD       (37) 

// 学习的命令词阈值门限，越小越灵敏，默认 35，最小可配置到 30；
#define CWSL_CMD_THRESHOLD          (35) 

// 学习时，重复的总次数, 建议范围 CWSL_REG_TIMES + 1、CWSL_REG_TIMES + 2、CWSL_REG_TIMES + 3
#define MAX_LEARN_REPEAT_NUMBER     (CWSL_REG_TIMES + 2)
...
{   //命令词ID     //学习提示播报音ID
    ///tag-insert-cwsl-cmd-learn-by-id-functioncode-pos-7
    //cwsl_cmd_learn_by_idfunction
    {11,             10196},
    {12,             10197},
    {13,             10198},
    {14,             10199},
...
{    
    ...
    if (word_type == WAKEUP_WORD)
    {
        ...
        // 播放提示音 "开始学习唤醒词"
        prompt_play_by_cmd_id(CWSL_REG_WAKE_UP, -1, cwsl_play_done_callback_with_start_record, false);
        #else
...
int on_cwsl_delete_successed()
{
    ...
    if (g_reg_status.reg_flag)  //注册指令已满时，删除成功后，再注册
    {
        ...
        if (cwsl_app.word_type == CMD_WORD)
        {
...
void cwsl_app_reg_word_new(cwsl_word_type_t word_type, uint8_t reg_cmd_index)
{
    if (cwsl_app.app_mode == CWSL_APP_REC)
    {
        cwsl_app.word_type = word_type;
        if (WAKEUP_WORD == word_type)
        {
            ...
            else
            {
                ...
                prompt_play_by_cmd_id(CWSL_TEMPLATE_FULL, -1, cwsl_play_done_callback_default, true);
                #endif
                cwsl_delete_word(-1, -1, WAKEUP_WORD); // 唤醒词已经注册满了，需要删除成功后，再注册
...
uint32_t cwsl_app_process_asr_msg(sys_msg_asr_data_t *asr_msg, cmd_handle_t *cmd_handle, uint16_t cmd_id)
{
    ...
    switch(cmd_id)
    {
        ///tag-insert-code-cwsl-register-by-cmd-pos-11
        //cwsl_register_by_cmd_function
        case CWSL_REG_WAKE_UP:
			cwsl_app_reg_word_new(WAKEUP_WORD, 0);
		break;
        case CWSL_DEL_WAKE_UP:
			cwsl_delete_word((uint32_t)-1, (uint16_t)-1,WAKEUP_WORD);
		break;
        case CWSL_REG_11:
			cwsl_app_reg_word_new(CMD_WORD, 0);
		break;
        case CWSL_REG_12:
			cwsl_app_reg_word_new(CMD_WORD, 1);
		break;
        case CWSL_REG_13:
			cwsl_app_reg_word_new(CMD_WORD, 2);
		break;
        case CWSL_REG_14:
			cwsl_app_reg_word_new(CMD_WORD, 3);
		break;
        case CWSL_DEL_11:
		...
		break;
        case CWSL_DEL_13:
			cwsl_delete_word(reg_cmd_list[2].reg_cmd_id, 0,CMD_WORD);
		break;
        case CWSL_DEL_14:
			cwsl_delete_word(reg_cmd_list[3].reg_cmd_id, 0,CMD_WORD);
		break;
